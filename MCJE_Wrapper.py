...
jar_name = config.get('settings', 'jar_name')
...
def start():
    ...
    try:
        server = subprocess.Popen(['java', '-Xms8G', '-Xmx20G', '-jar', server_path + '/' + jar_name, 'nogui'], stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError:
        logging.error("Server jar not found.")
        return
    mkdir()
